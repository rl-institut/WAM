...
from wam.settings import (
    BASE_DIR,
    WAM_EXCHANGE_ACCOUNT,
    WAM_EXCHANGE_EMAIL,
    WAM_EXCHANGE_PW,
)
...
class FeedbackView(FormView):  # pylint: disable=too-many-ancestors
    ...
    def __init__(self, app_name=None, intro_text=None):
        """
        Parameters
        ----------
        app_name : :obj:`str`
    ...
    def form_valid(self, form):
        ...
        success = send_email(to_email=self.to_email, subject=subject, message=body)
        if success:
    ...
    def get(self, request, *args, **kwargs):
        ...
        if (
            WAM_EXCHANGE_ACCOUNT is None
            or WAM_EXCHANGE_EMAIL is None
            or WAM_EXCHANGE_PW is None
        ):
            err_msg = (
                "Feedback-Form - Konfigurationsfehler: "
    ...
    def prepare_message(self, **kwargs):
        subject = f"Nachricht über WAM Feedback-Formular: " f"App {self.app_name}"
        body = (
            f"Sie haben eine Nachricht über das Feedback-Formular der WAM "
            f"erhalten.\n\n"
            f"App: {self.app_name}\n"
