...
RLI_THEME = {
    ...
    "plotOptions": {
        "series": {"dataLabels": {"style": {"fontWeight": None, "textOutline": None}}}
    },
}
...
class Highchart(HC):
    ...
    def add_pandas_data_set(self, data, series_type=None, **kwargs):
        ...
        if isinstance(data, pandas.Series):
            self.add_data_set(data.values.tolist(), series_type, data.name, **kwargs)
        ...
        if (
            ...
            and series_type != "scatter"
        ):
            self.set_options("xAxis", {"categories": data.index.values.tolist()})
